...
class Hkyh extends Component
{
    ...
    public $RETURN_ADMIN_URL; //  平台回调地址

    public $downloadUrl; // 对账下载地址
        ...
        if($privateKey){
            ...
            if (openssl_sign($reqDataJson, $sign, $privateKey, OPENSSL_ALGO_SHA1)) {
                ...
                if($serviceInfo){
                    ...
                    }else if($serviceInfo['type']=='download'){
                        return self::createHttpResponsePOST($this->downloadUrl, $httpResponse);  //直连接口
                    }else{
    ...
    function serviceNameInfo($name){
        ...
        //网关接口
        $gateway = ['PERSONAL_REGISTER_EXPAND','UNBIND_BANKCARD','PERSONAL_BIND_BANKCARD_EXPAND','RESET_PASSWORD','MODIFY_MOBILE_EXPAND','ACTIVATE_STOCKED_USER','RECHARGE','WITHDRAW','USER_AUTHORIZATION','USER_PRE_TRANSACTION','ENTERPRISE_BIND_BANKCARD'];

        //直连接口
        $service = ['DIRECT_RECHARGE','CONFIRM_WITHDRAW','CANCEL_WITHDRAW','ESTABLISH_PROJECT','MODIFY_PROJECT','CANCEL_PRE_TRANSACTION','SYNC_TRANSACTION','ASYNC_TRANSACTION','DEBENTURE_SALE','CANCEL_DEBENTURE_SALE','USER_AUTO_PRE_TRANSACTION',
            'UNFREEZE_TRADE_PASSWORD','QUERY_USER_INFORMATION','QUERY_TRANSACTION','QUERY_PROJECT_INFORMATION','CONFIRM_CHECKFILE'];

        $download = ['DOWNLOAD_CHECKFILE'];
