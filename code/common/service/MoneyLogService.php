...
class MoneyLogService extends BaseService{
    ...
    /**
     *根据member_id获取借款人昨天获取的放款金额
     * @author lijunwei
     */
    public static function getCreditByMemberId($member_id=null , $type = 'all'){
        if($member_id==null){
            return '0';
        }
        $query  = QfbMoneyLog::find();
        ...
        $query->andWhere(['in','action',[ MoneyEnum::ACTION22 ]]);
    ...
    /**
     *根据member_id获取借款人总共获取的放款金额
     * @author lijunwei
     */
    public static function getCreditByCount($member_id=null){
        if($member_id==null){
            return '0';
        }
        $query  = QfbMoneyLog::find();
    ...
    public static function  getFixInvestment($member_id){
        ...
        $query->where('member_id=:member_id and status not in (0,4)', [':member_id'=>$member_id]);
