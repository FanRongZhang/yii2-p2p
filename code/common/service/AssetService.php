...
class AssetService extends BaseService
{
    ...
    public function createByProject($data)
    {
        ...
        // 记录日志
        $fileName = "CJ_".$serviceName.'_DIRECT.log';
        $content = "创建标的操作   执行时间：".date("Y-m-d H:i:s", time())."   产品id：".$data->id."   请求数据：".json_encode($reqData)."\r\n";
    ...
    /*
    public function changeProjectState($data){
        ...
        // 请求流水号
        $reqData['requestNo'] = $this->getBindSn('MP');
        $reqData['projectNo'] = $data['sn'];
    ...
    public function preTransaction($data)
    {
        ...
        // 页面回跳 URL
        $reqData['redirectUrl'] = isset($data['pc'])? $hkyh->RETURN_PC_URL : $hkyh->RETURN_URL; //页面回跳 URL --必填*/
    ...
    public function batchTrato($data, $product)
    {
        ...
        if(count($data) != count($data, 1)){
            foreach ($data as $key => $value) {
                ...
                $details[$key]['income'] = $value['pay_money']*($product['year_rate']/100)*$product['invest_day']/Tool::yearDay();  //利息
            }
        }else{
