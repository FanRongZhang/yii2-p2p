...
class OrderFixService extends BaseService
{
    ...
    public function buyMemberList($product_id, $page = 1, $limit = 20)
    {
        ...
        $list = $this->findBySql(sprintf("
			...
			WHERE `qfb_order_fix`.`product_id` = %d AND `qfb_order_fix`.`status` IN (1,2,3) order by `qfb_order_fix`.id desc limit %d,%d ", $product_id, ($page - 1) * $limit, $limit));
        $tmp = [];
        if (count($list) > 0) {
    ...
    public function buyLimit($product_id, $member_id, $money){

        $product = QfbProduct::find()->select('id,max_money')->where(['id'=>$product_id])->one();

        $model = new $this->_className;

        $order_fix_money_sum = $model->find()->select('money')->where('product_id=:product_id and member_id=:member_id and status in (1,2,3) ', [':product_id'=>$product_id, ':member_id'=>$member_id])->sum('money');

        if(($order_fix_money_sum+$money) > $product->max_money){
            $this->addMessage('status', '购买该理财产品已达到上限');
            return false;
        }
    ...
    public function doSaveByMoney($params)
    {
        ...
        // 验证用户购买产品是否超过上限金额
        if(!$this->buyLimit($params['product_id'], $params['member_id'], $params['money']))
            return false;
...
//                    }
    ...
    public function create($params)
    {
        ...
        $this->setYearRate();
        $profit_money = Tool::moneyCalculate(Tool::moneyThousands(Tool::interestDay($this->model->money, $this->model->year_rate/100))*$this->productService->model->invest_day);
        $this->model->profit_money = Tool::moneyCalculate($profit_money);
        $this->model->end_time = 0;
        $this->model->next_profit_time = 0;
        $this->model->option_time = time();
        $this->model->option_status = 30;
        $this->model->day_interest = Tool::moneyThousands(Tool::interestDay($params['money'], $this->model->year_rate/100));
        ...
        //     $this->model->bank_id   = $params['bank_id'];
        // } else {
            // 海口银行支付，需要回调处理 订单支付成功，更改支付状态，标的到期时间，下次分润时间
    ...
    public function validNewer($productModel = null)
    {
        ...
        $is_new_user = $this->memberService->isNewerUser($this->member_id);
        if(!$is_new_user){
