...
//            \Yii::$app->set('db',yii::$app->components['dm_qfb']);
    ...
    /**
     * 判断是否新手-新加  李冠  临时解决 $this->getMemberID() 获取不到member_id
     * [isNewerUser description]
     * @return boolean [description]
     */
    public function isNewerUser($member_id){

        if($this->model!== null){
            return $this->model['is_newer']?true:false;
        }
        $model = new $this->_className;
        $this->model=$model::find()->where(['=','id',$member_id])->one();
        return $this->model['is_newer']?true:false;
    }
    /**
     * @param $member_id
