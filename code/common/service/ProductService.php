...
class ProductService extends BaseService{
	...
	public function detail($id){
		...
		$model=$model::find()->joinWith('product_agreement')->joinWith('product_detail')
		->select("qfb_product.id,qfb_product.end_time,qfb_product.product_name,qfb_product.min_money,qfb_product.max_money,qfb_product.has_money,qfb_product.stock_money,
			...
			qfb_agreement.title,qfb_agreement.content,qfb_agreement.pic_url")
	...
	public function getAllList($type,$sort,$page,$limit,$is_index="",$stock_money="",$category_id="", $is_app=false) {
		...
		$query->where(['in','qfb_product.status',array(1,2,5,6,7,8)]);
    ...
    public function  getProductByCid($cid){
		...
		$query = QfbProCategory::find()->select("qfb_product.id")->orderBy('id desc')->asArray()->joinWith("product")
			->andWhere(['=','qfb_product.category_id',$cid]);

		// 非新手产品展示原规则
		if($cid != 1){
			$query = $query->andWhere(['=','qfb_product.status',1]);
		}else{
			$query = $query->andWhere(['in','qfb_product.status',[1,2,5,6,7,8]]);
		}

		return $query->andWhere(['=','qfb_product.is_hidden',0])->one();

		/*
        return QfbProCategory::find()->select("qfb_product.id")->orderBy('id desc')->asArray()->joinWith("product")
            ->andWhere(['=','qfb_product.category_id',$cid])
			->andWhere(['=','qfb_product.status',1])
			->andWhere(['=','qfb_product.is_hidden',0])
            ->one();*/
    }
