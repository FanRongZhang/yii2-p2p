...
namespace common\extension\curl;
...
class Curl
{
     ...
     */
    public function exec($ch = null, $type='post')
    {
        if ($ch === null) {
        ...
        if($type == 'get'){
            if($this->httpStatusCode == '200'){
                list($this->requestHeaders, $this->response) = explode("\r\n\r\n", $this->response, 2);
            }
        }
    ...
    public function get($url, $data = array())
    {
        ...
        //$this->setOpt(CURLOPT_RETURNTRANSFER, 1);CURLINFO_HEADER_OUT
        $this->setOpt(CURLINFO_HEADER_OUT, true);
        $this->setOpt(CURLOPT_SSL_VERIFYPEER, false);
        $this->setOpt(CURLOPT_SSL_VERIFYHOST, false);
        return $this->exec(null, 'get');
    }
