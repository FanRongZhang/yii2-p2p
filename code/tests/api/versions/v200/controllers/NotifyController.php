    ...
    public function actionHkyh()
    {
        ...
        switch($data['serviceName']){
            ...
            case 'RECHARGE':
                ...
                if($result['code'] == code::HTTP_OK)
                    $status = 'success';
                return $this->redirect(['/v200/notify/hkyh-return','status'=>$status, 'order_id'=>$result['data']['requestNo'], 'type'=>'hkyh-userPay','msg'=>$result['msg']]);                       // 充值回调业务处理
                break;
            // 用户预处理
            ...
            case 'WITHDRAW':
                ...
                if($result['code'] == code::HTTP_OK)
                    $status = 'success';
                return $this->redirect(['/v200/notify/hkyh-return','status'=>$status,  'order_id'=>$result['data']['sn'], 'type'=>'hkyh-withdraw','msg'=>$result['msg']]);
                break;
            case 'PLATFORM_PRE_TRANSACTION':
                $result = $hkyhService->hkyhRepayment($data);
                if($result['code'] == code::HTTP_OK)
                    $status = 'success';
                return $this->redirect(['/v200/notify/hkyh-return','status'=>$status,'type'=>'hkyh-withdraw','msg'=>$result['msg']]);
            break;
            // 修改交易密码
            case 'RESET_PASSWORD':
