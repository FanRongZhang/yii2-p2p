...
//            \Yii::$app->set('db',yii::$app->components['dm_qfb']);
            ...
            if ($member->member_type != 1)
                return ['code' => Code::COMMON_ERROR_CODE, 'msg' => '借款人账户，操作PC端登录'];
            ...
            // 更新会员信息
            $update_member_arr['channel_id'] = $post['channelId'];
            QfbMember::updateAll($update_member_arr, ['id'=>$member['id']]);
    ...
    public function actionHkyhRegister()
    {
        ...
        if($getHkyhUser['code'] != code::HTTP_OK){
            ...
            // 标识异常（提交请求状态）
            $update_member_arr['is_dredge'] = 9;
            $update_member_arr['option_time'] = time();
            QfbMember::updateAll($update_member_arr, ['id'=>$member_id]);
    ...
    public function actionHkyhResetPassword(){
        ...
        $content = '修改密码操作                 请求数据：'.json_encode($reqData)."\r\n";
        LogService::hkyh_write_log($fileName, $content);
