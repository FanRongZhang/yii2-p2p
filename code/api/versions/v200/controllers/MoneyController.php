...
class MoneyController extends BaseController{
    ...
    public function actionUserPay(){
        ...
        if(empty($member_id) )
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'nologin','type'=>'hkyh-userPay','msg'=>'未登录']);

        if (empty($params['money']) || empty($params['bank_id'])){
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'参数有误']);

        }
        if ($params['money'] < 1) {
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'单笔充值最小为1元']);
        }
        ...
        if (empty($bank_limit)) {
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'未能识别的银行卡']);
        }

        if ($params['money'] > $bank_limit['one_trade']) {
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'单笔交易限额为'.$bank_limit['one_trade'].'元']);
        }
        ...
        if ($params['money'] > $residue) {
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'当日剩余交易额度为'.$residue.'元']);
        }
        ...
        if ($bank_limit['trade_num'] != 0) {
            ...
            if ($count >= $bank_limit['trade_num']) {
                return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'已超过当月限制交易次数']);
            }
        }
        //单月交易限额
        if ($bank_limit['month_trade'] != 0) {
            ...
            if ($params['money'] > $residue) {
                return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>'当月剩余交易额度为'.$residue.'元']);
            }
        }
        ...
        // 块钱通道
            ...
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error','type'=>'hkyh-userPay','msg'=>'支付通道有误']);
        }
        ...
        if ($is_dredge==1) {
            ...
            if($getHkyhUser['code'] != code::HTTP_OK){
                ...
                $content = '绑卡注册操作                 请求数据：'.json_encode($reqData)."\r\n";
                LogService::hkyh_write_log($fileName, $content);
            ...
            if($result['code'] == code::HTTP_OK){
                ...
                $fileName = $serviceName."-REQUEST.log";
                $content = '充值操作                 请求数据：'.json_encode($reqData)."\r\n";
                LogService::hkyh_write_log($fileName, $content);
                ...
                return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0','type'=>'hkyh-userPay', 'msg'=>$result['msg']]);
            }
        }
    ...
    public function actionMymoney(){
        ...
        $member_data = MemberService::findModelById($member_id,'','is_dredge');
        if($member_data->is_dredge == '1') $card_no = 1;
    ...
    public function actionWithdraw()
    {
        ...
        if($status == false){
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'仅工作日'.$system['open_start_time'].':00-'.$system['open_end_time'].':00支持提现']);
        }
        ...
        if(!isset($params['money']) || !preg_match('/^[0-9]+(.[0-9]{1,2})?$/', $params['money']))
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'参数错误']);

        $memberMoney = QfbMemberMoney::find()->where(['member_id'=>$member_id])->asArray()->one();
        if($params['money'] > $memberMoney['money']){
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'余额不足']);
        }

        if($params['money'] > $system['per_money']){
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'单笔最大提现金额为'.$system['per_money'].'元']);
        }

        //获取当前用户绑卡所属银行信息
        $bankinfo=QfbBank::findOne(['member_id'=>$member_id, 'is_del'=>0]);
        //查询默认通道信息
        $channel = QfbChannel::find()->where(['is_default'=>1])->one();
        //查询用户当前绑卡银行限额
        $bank_limit=QfbBankLimit::find()->where(['bank_abbr'=>$bankinfo['bank_abbr'], 'pt_type'=>$channel['id']])->one();
        ...
        $nowMoney = QfbOrder::find()->where(['type'=>2, 'sorts'=>1, 'member_id'=>$member_id])->andFilterWhere(['in', 'is_check', [1,3,5]])->andFilterWhere(['between','create_time', $begintime, $endtime])->sum('price');

        if(($nowMoney+$params['money']) > $system['day_money']){
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'单日最大提现金额为'.$system['day_money'].'元']);
        }
        ...
        $bank = QfbBank::findOne(['member_id'=>$member_id]);
        ...
        //计算提现手续费
        if( $params['money'] >= $system['min_money']){
            if($system['fast_rate'] > 0){
                $reqData['commission'] = Tool::moneyPlatform($params['money']*$system['fast_rate']/100);
            }
        }else{
            if($system['money_fee'] != 0)
                $reqData['commission'] = Tool::moneyPlatform($system['money_fee']);
            if($params['money'] <= $system['money_fee']){
                return $this->redirect(['/v200/notify/hkyh-return','status'=>'error', 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'提现金额不能低于'.$system['money_fee'].'元']);
            }
        }
        ...
        if (!$order->save()) {
            return $this->redirect(['/v200/notify/hkyh-return','status'=>'error' , 'order_id'=>'0', 'type'=>'hkyh-withdraw', 'msg'=>'系统异常']);
        }
    ...
    public function actionRule(){
        ...
        if($params['type'] == 2){
            ...
            $tips = '1.'.$setting['close_content'].'；<br>2.快速提现手续费率'.$setting['fast_rate'].'%；<br>'.
                '3.每笔提现低于'.$setting['min_money'].'，固定收取'.$setting['money_fee'].'元手续费。大于等于'.$setting['min_money'].'元的按费率收取手续费，每日最高可提现'.$setting['day_money'].'元。';
            $list = [
                [
                    'rate' => $setting['fast_rate'],
        ...
        // 充值规则
            ...
            //获取当前用户绑卡所属银行信息
            $bankinfo = QfbBank::find()->where(['member_id'=>$member_id, 'is_del'=>'0'])->one();
            ...
            $countmoney=QfbOrder::find()
                ->where(['sorts'=>1,'type'=>1,'member_id'=>$member_id])
                ->andFilterWhere(['in','is_check',[0,1,3,5]])
                ->andFilterWhere(['between','create_time', $begintime, $endtime])
                ->sum('price');
            ...
            $limit = 0;
            $limit_money = 0;
            $tips = "1.单笔最多充值".$bank_limit['one_trade']."元  2.今日剩余充值额度为".$residue.'元';
            $list = [];
