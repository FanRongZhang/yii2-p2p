        ...
        <div class="con-clo-main paddnones">
                <div class="con-list-box">
                    <div class="con-border-box">
                        ...
                        <?php if ($model['category_id'] == 1 || $model['is_newer'] == 1) { ?>  
                    ...
                    <div class="clearfix">
                        <div class="pr-prdetails-box fl">
                            <div class="con-prdetails">
                                <div class="con-main-clounmt clearfix">
                                    ...
                                    <div class="fl per-box ied2 ied10">
                            ...
                            <div class="con-prdetails2 padb">
                                ...
                                <div class="clearfix con-main-clounmt2 mar30">
                                    <div class="fl"><i class="officicon pr-in1"></i><span class="pad25 gray3">
                                    ...
                                    <div class="fl marleft"><i class="officicon pr-in2"></i><span class="pad25 gray3">起投金额<?php echo $model['min_money'] ?>元</span></div>
                                </div>
                            </div>
                        </div>
                        ...
                        <?php if (empty($member_data) || $member_data->is_dredge != 1) {?>
                            <div class="pr-bank-con" style="display:block">
                                ...
                                <div class="mar30">
                                    <a class="btn btnmains btn-wdetials" href="
                                    
                                    <?php if(empty($member_data)) {?>
                                    /login/login/login
                                    <?php }else{?>
                                    /login/login/reg-bank?member_type=<?php echo $member_data->member_type ?>
                                    <?php }?>
                                    ">同意协议并投资</a>
                                    <?php if ($model['category_id'] == 1 || $model['is_newer'] == 1) { ?> 
                                    <p class="gray2 mar6">限未成功投资过的新用户</p>
                                    <?php }?>
                                </div>
                            </div>
                        <?php }else{ ?>
                            <div style="display:block;">
                                <div class="pr-inp-box1">
                                    ...
                                    <div class="clearfix pad-botm">
                                        ...
                                        <div class="fl xitoi-tit">限投<?php echo $ceiling ?>元</div>
                                ...
                                <div class="btmarts">
                                    ...
                                    <div class="smart-form td-pr-check gray2">
                                        <label class="checkbox fl">
                                            <input type="checkbox" name="RegisterForm[agreement]" <?php if ($model['can_money_ticket']==0){echo " disabled='disabled'";} ?> id="CheckboxGroup1_0">
                                            <i></i>
                                        </label>
                    ...
                    <table width="100%">
                        <tbody>
                            ...
                            <tr>
                                ...
                                <td class="td2 black1">
                                <?php if($model['profit_day'] ==10){?>
                                    投资日+<?php echo $model['invest_day'] ?>天
                                    <?php }elseif($model['profit_day'] ==11){ ?>
                                    投资日+<?php echo 1+$model['invest_day'] ?>天
                                    <?php }elseif($model['profit_day'] ==20){ ?>
                                    满标日+<?php echo $model['invest_day'] ?>天
                                    <?php }elseif($model['profit_day'] ==21){ ?>
                                    满标日+<?php echo 1+$model['invest_day'] ?>天
                                <?php }?>
                                </td>
                            </tr>
                            <tr>
                            ...
                            <tr>
                                ...
                                <?php foreach ($model['product_agreement'] as $key => $v) { ?>
                                    <?php if($v['title'] == '借款协议'){ ?>
                                        <a href="/index/index/loanpdf?id=<?php echo $model['id']; ?>">
                                            《<?php echo $v['title'] ?>》
                                        </a>
                                    <?php }else{ ?>
                                        <a href="/index/index/xy?id=<?php echo $v['id'] ?>">
                                            《<?php echo $v['title'] ?>》
                                        </a> 
                                    <?php } ?>
                                <?php }?>
    ...
    <div class="bombbox">
        ...
        <div class="bombbox5">
            <div class="bombbox1 clearfix">
                ...
                <div class="fl"><?php if(!empty($member_bank)){ echo  $member_bank['name'] ;}?></div>
                <div class="fr">尾号：<?php if(!empty($member_bank)){ echo substr($member_bank['no'],-4);} ?></div>
            </div>
            <div class="bombbox6">注：单笔限额<?php echo $one_trade ?>元，每日限额<?php echo $day_trade ?>元</div>
        </div>
...
//获取用户信息
var member_id=<?php if(!empty($member_data)){ echo $member_data->id;} ?>
//用户是否新手
var is_newer=<?php  if(!empty($member_data)){ echo $member_data->is_newer;} ?>
//用户身份
var member_type=<?php  if(!empty($member_data)){ echo $member_data->member_type;}?>
//产品是否新手
var pro_isnewer=<?php echo $model['is_newer'] ?>
//银行卡ID
var bank_id=<?php if(!empty($member_bank)){echo $member_bank->id;} ?>
//用户余额
var member_money=<?php echo $member_money ? $member_money : 0 ?>
<?php }?>
//产品ID
...
//当前用户投资上限金额
var ceiling=<?php echo intval($ceiling) ?>
...
//结束倒计时
addTimer("timer6", <?php if ($model['status'] != 1 || $model['endtime']<0){echo 0;}else{echo $model['endtime'];} ?>);
...
function income() {
    ...
    if (money > ceiling){
        $('.with-taste,.dm-popup-box').show();
        $("body").addClass("bodyshopcar");
        $('#message').html('投资上限金额为'+ceiling+'元')
        return false
    }
    ...
    var come= money*year_rate/100/365;
    var income=Math.floor(come * 1000) / 1000
    var income=Math.floor(income*invest_day*100)/100
...
function recharge() {
    ...
    money=money.replace(/\b(0+)/gi,"")
    var re = /^-?\d+\.?\d{0,2}$/;
    if (!re.test(money) || money < 1) {
       alert('充值金额错误');
       $('#money').val('')
       return false
    }
    if (money > <?php echo $residue ? $residue : 0?>) {
        alert('当日剩余交易金额为'+<?php echo $residue ? $residue : 0 ?>+'元');
        $('#money').val(<?php echo $residue ?>)
        return false
    }
    if (money > <?php echo $one_trade ? $one_trade : 0 ?>){
        alert('单笔交易限额为'+<?php echo $one_trade ? $one_trade : 0 ?>+'元');
        $('#money').val(<?php echo $one_trade ? $one_trade : 0 ?>)
        return false
    }
 ...
 $('.with-taste').hide();
    ...
    function invest(){
        ...
        location.href='/index/index/invest?money='+money+'&product_id='+product_id
    };
