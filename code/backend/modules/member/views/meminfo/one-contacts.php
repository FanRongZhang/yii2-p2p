    ...
    <?= GridView::widget([
        ...
        'columns' => [
            ...
            [ 'label'=>"推荐人账号",'content'=>function($data){
                    return !empty(QfbMember::find()->where(['id' => \Yii::$app->request->get('id')])->one()) ? QfbMember::find()->where(['id' => \Yii::$app->request->get('id')])->one()->account : "";
                }
            ],

            [ 'label'=>"推荐人姓名",'content'=>function($data){
                    return !empty(QfbMemberInfo::find()->where(['member_id' => \Yii::$app->request->get('id')])->one()) ? QfbMemberInfo::find()->where(['member_id' => \Yii::$app->request->get('id')])->one()->realname : "";
                }
            ],
            ...
            [ 'label'=>"活期贡献分润",'content'=>function($data){
                    $moneydetail = QfbMoneyDetail::find()->where(['member_id' => \Yii::$app->request->get('id'),'from_member_id' => $data->id,'money_type' => 1])->one();
